...
public class GunBase : MonoBehaviour
{
    ...
    [SerializeField]
    protected GameObject AmmoObj;   				// 弾丸オブジェクト
    protected AudioSource audioSource;              // 音の再生窓口
    [SerializeField]
    protected AudioClip ShotSound; 					// ショット音
    [SerializeField]
    protected Light Flash;							// マズルフラッシュのライト
	[SerializeField]
	public ParticleSystem Smoke;    				// マズルフラッシュエフェクト

    [SerializeField]
    protected int MaxAmmo;          				// 所持総弾数
    [SerializeField]
    protected int MaxMagazine;      				// マガジンの最大数
    ...
    [SerializeField]
    protected float ReloadTime = 1; 				// リロードする時間
    protected bool Next = true;                     // 次に攻撃する時間間隔(フラグ)
    [SerializeField]
    Vector3 Rotate;                 // 銃を取得した際の向きを指定
    [SerializeField]
    Rigidbody myRigidbody;          // 物理処理コンポーネントのキャッシュ
    [SerializeField]
